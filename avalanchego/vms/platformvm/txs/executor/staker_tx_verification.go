...
func verifyAddValidatorTx(
	...
	// For Flare, get limits based on the current timestamp
	minValidatorStake, maxValidatorStake, _, minDelegationFee, minStakeDuration, _, maxStakeDuration, minFutureStartTimeOffset, _, minStakeStartTime := GetCurrentInflationSettings(currentTimestamp, backend.Ctx.NetworkID, config)
...
func verifyAddSubnetValidatorTx(
	...
	// Disable subnet validators on Flare network
	if backend.Ctx.NetworkID == constants.FlareID || backend.Ctx.NetworkID == constants.CostwoID || backend.Ctx.NetworkID == constants.StagingID || backend.Ctx.NetworkID == constants.LocalFlareID {
		return ErrStakeTooLong
	}
...
func verifyAddDelegatorTx(
	...
	// For Flare, get limits based on the current timestamp
	_, maxValidatorStake, minDelegatorStake, _, _, minDelegateDuration, maxStakeDuration, minFutureStartTimeOffset, maxValidatorWeightFactor, _ := GetCurrentInflationSettings(currentTimestamp, backend.Ctx.NetworkID, config)
	...
	if backend.Config.IsApricotPhase3Activated(currentTimestamp) {
		maximumWeight = safemath.Min(maximumWeight, maxValidatorStake)
	}
