...
func (st *StateTransition) GetBlockTime() *big.Int {
	return new(big.Int).SetUint64(st.evm.Context.Time)
}
...
func (st *StateTransition) TransitionDb() (*ExecutionResult, error) {
	...
	if contractCreation {
		...
		if vmerr == nil && chainID != nil && timestamp != nil && st.evm.Context.Coinbase == common.HexToAddress("0x0100000000000000000000000000000000000000") {
			if GetStateConnectorIsActivatedAndCalled(chainID, timestamp, *msg.To) &&
				len(st.msg.Data) >= 36 && len(ret) == 32 &&
				bytes.Equal(st.msg.Data[0:4], SubmitAttestationSelector(chainID, timestamp)) &&
				binary.BigEndian.Uint64(ret[24:32]) > 0 {
				err = st.FinalisePreviousRound(chainID, timestamp, st.msg.Data[4:36])
				if err != nil {
					log.Warn("Error finalising state connector round", "error", err)
				}
			} else if GetGovernanceSettingIsActivatedAndCalled(chainID, timestamp, *msg.To) && len(st.msg.Data) == 36 {
				if bytes.Equal(st.msg.Data[0:4], SetGovernanceAddressSelector(chainID, timestamp)) {
					err = st.SetGovernanceAddress(chainID, timestamp, st.msg.Data[4:36])
					if err != nil {
						log.Warn("Error setting governance address", "error", err)
					}
				} else if bytes.Equal(st.msg.Data[0:4], SetTimelockSelector(chainID, timestamp)) {
					err = st.SetTimelock(chainID, timestamp, st.msg.Data[4:36])
					if err != nil {
						log.Warn("Error setting governance timelock", "error", err)
					}
				}
			} else if GetInitialAirdropChangeIsActivatedAndCalled(chainID, timestamp, *msg.To) && len(st.msg.Data) == 4 {
				if bytes.Equal(st.msg.Data[0:4], UpdateInitialAirdropAddressSelector(chainID, timestamp)) {
					err = st.UpdateInitialAirdropAddress(chainID, timestamp)
					if err != nil {
						log.Warn("Error updating initialAirdrop contract", "error", err)
					}
				}
			} else if GetDistributionChangeIsActivatedAndCalled(chainID, timestamp, *msg.To) && len(st.msg.Data) == 4 {
				if bytes.Equal(st.msg.Data[0:4], UpdateDistributionAddressSelector(chainID, timestamp)) {
	...
	if vmerr == nil && msg.To != nil && *msg.To == common.HexToAddress(GetPrioritisedFTSOContract(timestamp)) && st.initialGas <= GetMaxFTSOGasLimit(timestamp) {
		...
		actualGasPrice := st.evm.GasPrice
		...
		st.state.AddBalance(burnAddress, new(big.Int).Mul(new(big.Int).SetUint64(st.gasUsed()), st.evm.GasPrice))
	}
	// TODO: FLARE --- Ava code: st.state.AddBalance(st.evm.Context.Coinbase, new(big.Int).Mul(new(big.Int).SetUint64(st.gasUsed()), msg.GasPrice))

	// Call the daemon if there is no vm error
	if vmerr == nil && chainID != nil && (chainID.Cmp(params.FlareChainID) == 0 || chainID.Cmp(params.SongbirdChainID) == 0 || chainID.Cmp(params.CostwoChainID) == 0 || chainID.Cmp(params.StagingChainID) == 0 || chainID.Cmp(params.LocalFlareChainID) == 0) {
		log := log.Root()
...
func (st *StateTransition) gasUsed() uint64 {
	...
	// FLARE, TODO: 	return st.initialGas - st.gasRemaining
}
